...
/** \brief Obtiene el elemento seleccionado de la lista.
 *
 * \param this LinkedList* Puntero a la lista
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && (index > -1 && index < len))
    {
    	...
    	else
    	{
    		...
    		nodeAnterior->pNextNode = nodeAux->pNextNode;
    	}
    	free(nodeAux);
    	returnAux = 0;
    	this->size--;
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
    ...
    //pFunc(pElement1, pElement2) < 0 (El primero es menor)
    //pFunc(pElement1, pElement2) == 0 (Iguales)
    //pFunc(pElement1, pElement2) > 0 (El primero es mayor)
...
/** \brief Crea una nueva lista con los elementos filtrados.
 * \param pList LinkedList* Puntero a la lista
 * \param pFunc (*fn) [1] si paso el filtro o [0] si no paso el filtro
 * \return LinkedList* Retorna  (NULL) Error: si el puntero a la listas es NULL
                                o (si el filtro es NULL)
                         	 	(puntero a la nueva lista) Si ok
 */
